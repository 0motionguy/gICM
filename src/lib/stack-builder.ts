...
export function generateInstallCommand(selected: string[]): string {
  ...
  return `npx @clawdbot/cli add ${items.join(" ")} --yes`;
}
...
> Generated by ClawdBot://SEND - Vibecoding Marketplace
...
**Generated with ❤️ by ICM Motion**
Spawned from ClawdBot://SEND marketplace
`;
}
...
**Installs:** ${item.installs} • **Remixes:** ${item.remixes}
`;
...
**Installs:** ${item.installs || 0} • **Remixes:** ${item.remixes || 0}
`;
  }
}
...
echo "🚀 Setting up ClawdBot Stack..."
...
if [ ! -f .env ]; then
    ...
    cp .env.example .env
    echo "⚠️  Please fill in environment variables in .env"
fi
...
function generateEnvExample(items: RegistryItem[]): string {
  ...
  return `# ClawdBot Stack Environment Variables
