...
program.name("gicm-product").description("gICM Product Engine - Automated product development").version("1.0.0");
program.command("start").description("Start the product engine").option("--no-discovery", "Disable discovery").option("--no-auto-build", "Disable auto-building").action(async (options) => {
  console.log(chalk.cyan.bold("\n\u{1F527} gICM Product Engine\n"));
  const engine = new ProductEngine({
    enableDiscovery: options.discovery,
    discoveryInterval: process.env.DISCOVERY_INTERVAL || "0 */6 * * *",
    // Every 6 hours
...
program.command("discover").description("Run discovery cycle").action(async () => {
  console.log(chalk.cyan.bold("\n\u{1F50D} Running Discovery\n"));
  const engine = new ProductEngine({
  ...
  try {
    ...
    if (opportunities.length > 0) {
      ...
      for (const opp of opportunities.slice(0, 5)) {
        const icon = opp.priority === "high" || opp.priority === "critical" ? "\u{1F534}" : "\u{1F7E1}";
...
program.command("backlog").description("Show backlog").action(async () => {
  console.log(chalk.cyan.bold("\n\u{1F4CB} Product Backlog\n"));
  const engine = new ProductEngine({
...
program.command("build-agent").description("Build a new agent interactively").action(async () => {
  console.log(chalk.cyan.bold("\n\u{1F916} Agent Builder\n"));
  ...
  const spec = {
    name: answers.name.replace(/\s+/g, "") + (answers.name.endsWith("Agent") ? "" : "Agent"),
    slug: answers.name.toLowerCase().replace(/\s+/g, "-").replace(/agent$/, "") + "-agent",
    description: answers.description,
    category: answers.category,
    capabilities: answers.capabilities.split(",").map((c) => c.trim()).filter(Boolean),
    inputs: [
      {
        name: "input",
...
program.command("status").description("Show engine status").action(() => {
