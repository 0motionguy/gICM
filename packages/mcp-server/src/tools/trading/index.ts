...
export const tradingTools = {
  get_market_data: {
    description:
      "Get real-time market data for a cryptocurrency token from DexScreener and CoinGecko",
    parameters: {
      token: {
        type: "string",
  ...
  analyze_token: {
    description:
      "Run multi-agent AI analysis on a token using famous investor personas (Buffett, Burry, etc.) and crypto-native strategies",
    parameters: {
      token: {
        type: "string",
      ...
      mode: {
        type: "string",
        description:
          "Analysis mode: 'full' (all agents), 'fast' (quick screening), 'degen' (memecoin focus)",
  ...
  quick_signal: {
    description:
      "Get a quick buy/sell/hold signal for a token with minimal analysis - good for screening",
    parameters: {
      token: {
        type: "string",
