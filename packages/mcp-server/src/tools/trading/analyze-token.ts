...
const AI_HEDGE_FUND_URL =
  process.env.AI_HEDGE_FUND_URL || "http://localhost:8001";
...
export async function analyzeToken(
  ...
  try {
    ...
    const bullish = signals.filter(
      (s: any) => s.action === "bullish"
    ).length;
    const bearish = signals.filter(
      (s: any) => s.action === "bearish"
    ).length;

    let sentiment = "neutral";
    if (bullish > bearish * 2) sentiment = "very_bullish";
    else if (bullish > bearish) sentiment = "bullish";
    else if (bearish > bullish * 2) sentiment = "very_bearish";
    else if (bearish > bullish) sentiment = "bearish";

    const avgConfidence =
      signals.length > 0
        ? signals.reduce((sum: number, s: any) => sum + s.confidence, 0) /
          signals.length
        : 0;
    ...
    return {
      ...
      agentSignals: signals.map((s: any) => ({
        agent: s.agent,
        action: s.action,
...
async function fallbackAnalysis(
  ...
  try {
    const response = await fetch(
      `${DEXSCREENER_BASE}/dex/search?q=${encodeURIComponent(token)}`
    );
    const data = await response.json();
    ...
    return {
      ...
      action,
      conviction: "low",
      summary: `Basic analysis for ${token}: ${change24h.toFixed(
        1
      )}% 24h change, $${volume.toLocaleString()} volume, $${liquidity.toLocaleString()} liquidity. Note: AI Hedge Fund API not available for full analysis.`,
      agentSignals: [
        {
          ...
          action: sentiment.includes("bullish")
            ...
            : "neutral",
          confidence: 50,
          reasoning: `Price changed ${change24h.toFixed(
            1
          )}% in 24h with $${liquidity.toLocaleString()} liquidity`,
        },
      ],
    };
  } catch (e) {
