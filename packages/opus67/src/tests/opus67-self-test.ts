...
import { AgentSpawner, AgentTask, AgentResult, createSpawner } from '../benchmark/agent-spawner.js';
import { TokenTracker, tokenTracker, ModelName } from '../benchmark/token-tracker.js';
import { MetricsCollector, metricsCollector, BenchmarkMetrics } from '../benchmark/metrics-collector.js';
import { GraphitiMemory, createMemory } from '../memory/graphiti.js';
import { ModeSelector, getAllModes, getMode, detectMode, ModeName } from '../mode-selector.js';
import { loadSkills, loadRegistry, formatSkillsForPrompt, LoadContext, Skill } from '../skill-loader.js';
...
interface DependencyGraph {
  nodes: Map<string, { file: string; imports: number; exports: number; complexity: number }>;
...
interface SelfTestReport {
  ...
  successCriteria: Record<string, { target: string; actual: string; passed: boolean }>;
...
function extractFunctions(content: string): FunctionInfo[] {
  ...
  let currentFunction: { name: string; startLine: number; indent: number; isAsync: boolean; isExported: boolean; params: number } | null = null;
  let braceCount = 0;
...
function buildDependencyGraph(files: FileAnalysis[]): DependencyGraph {
  ...
  for (const file of files) {
    ...
    if (!importedFiles.has(file.relativePath) &&
  ...
  for (const file of files) {
    ...
    const exports = graph.edges.filter(e => e.to === file.relativePath && e.type === 'internal').length;
 ...
 */
function generateArchitectureDiagram(files: FileAnalysis[], graph: DependencyGraph): string {
  ...
  const coreModules = files.filter(f =>
 ...
 */
function generateRecommendations(files: FileAnalysis[], graph: DependencyGraph): string[] {
...
export class OPUS67SelfTest extends EventEmitter<SelfTestEvents> {
  ...
  private async runDependencyAnalysis(): Promise<DependencyGraph> {
    ...
    const externalEdges = graph.edges.filter(e => e.type === 'external').length;
  ...
  private async runAgentSimulation(): Promise<{
    ...
    const insights = results.filter(r => r.success).map(r => r.output as string);
...
|-----------|--------|--------|--------|
${Object.entries(report.successCriteria).map(([name, { target, actual, passed }]) =>
...
${this.fileAnalyses
  ...
  .slice(0, 20)
  .map(f => `| ${f.relativePath} | ${f.lines} | ${f.functions.length} | ${f.classes.length} | ${f.complexity} |`)
  .join('\n')}
...
`;
  ...
  private generateReport(): SelfTestReport {
    ...
    const avgComplexity = this.fileAnalyses.length > 0
      ? this.fileAnalyses.reduce((sum, f) => sum + f.complexity, 0) / this.fileAnalyses.length
      : 0;
    const maxComplexityFile = this.fileAnalyses.length > 0
      ? this.fileAnalyses.reduce((max, f) => f.complexity > max.complexity ? f : max, this.fileAnalyses[0])
    ...
    const successCriteria: Record<string, { target: string; actual: string; passed: boolean }> = {
      'Files parsed': {
        target: '>50',
  ...
  async run(): Promise<SelfTestReport> {
    ...
    const phase1: TestPhase = { name: 'Deep File Scan', description: 'Parse TypeScript files', status: 'pending' };
    ...
    const phase2: TestPhase = { name: 'Metrics Calculation', description: 'Cyclomatic complexity', status: 'pending' };
    ...
    const phase3: TestPhase = { name: 'Dependency Analysis', description: 'Build import graph', status: 'pending' };
    ...
    const phase4: TestPhase = { name: 'Component Verification', description: 'Skills, Modes, Memory', status: 'pending' };
    ...
    // Phase 5: Agent Simulation
    const phase5: TestPhase = { name: 'Agent Simulation', description: '20 parallel agents', status: 'pending' };
    ...
    const phase6: TestPhase = { name: 'Report Generation', description: 'Write Markdown file', status: 'pending' };
  ...
  private printReport(report: SelfTestReport): void {
    ...
    console.log('╠══════════════════════════════════════════════════════════════════╣');
    console.log(`║  Duration:           ${(report.duration / 1000).toFixed(2).padEnd(10)}seconds                        ║`);
