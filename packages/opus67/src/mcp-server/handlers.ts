 ...
 */
export function handleGetSkill(ctx: HandlerContext, args: ToolArgs): HandlerResult {
 ...
 * List all skills
 */
export function handleListSkills(ctx: HandlerContext, args: ToolArgs): HandlerResult {
  const category = args.category;
  const filtered = category
    ? ctx.skills.filter(s => s.category.toLowerCase() === category.toLowerCase())
    : ctx.skills;
 ...
 */
export function handleDetectSkills(ctx: HandlerContext, args: ToolArgs): HandlerResult {
 ...
 * Get mode details
 */
export function handleGetMode(ctx: HandlerContext, args: ToolArgs): HandlerResult {
 ...
 */
export function handleListMcps(ctx: HandlerContext, args: ToolArgs): HandlerResult {
  const category = args.category;
  const filtered = category
    ? ctx.mcpConnections.filter(m => m.category?.toLowerCase() === category.toLowerCase())
    : ctx.mcpConnections;
 ...
 */
export function handleGetContext(ctx: HandlerContext, args: ToolArgs): HandlerResult {
 ...
 */
export function handleToolCall(name: string, args: ToolArgs, ctx: HandlerContext): HandlerResult {
  switch (name) {
    case 'opus67_boot': return handleBoot(ctx);
    case 'opus67_get_skill': return handleGetSkill(ctx, args);
    case 'opus67_list_skills': return handleListSkills(ctx, args);
    case 'opus67_detect_skills': return handleDetectSkills(ctx, args);
    case 'opus67_get_mode': return handleGetMode(ctx, args);
    case 'opus67_list_modes': return handleListModes(ctx);
    case 'opus67_list_mcps': return handleListMcps(ctx, args);
    case 'opus67_get_context': return handleGetContext(ctx, args);
    case 'opus67_status': return handleStatus(ctx);
    default:
