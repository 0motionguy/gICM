...
function parseIdlType(type) {
    if (typeof type === 'string')
        return type;
    if (typeof type === 'object' && type !== null) {
        const t = type;
        if ('vec' in t)
            return `Vec<${parseIdlType(t.vec)}>`;
        if ('option' in t)
            return `Option<${parseIdlType(t.option)}>`;
        if ('array' in t) {
            const arr = t.array;
...
function formatInstructionDocs(idl, ixName) {
...
async function listLoadedIdls() {
    return Array.from(loadedIDLs.values());
}
async function getInstructionInfo(programName, instructionName) {
...
async function generateInstruction(programName, instructionName, args, accounts) {
    const loaded = loadedIDLs.get(programName);
    if (!loaded) {
        throw new Error(`Program ${programName} not loaded. Use load_idl first.`);
    }
...
async function simulateTransaction(programName, instructionName, args, accounts) {
    try {
        const loaded = loadedIDLs.get(programName);
        if (!loaded) {
        ...
        const ix = await program.methods[instructionName](...ixArgs)
            .accounts(ixAccounts)
            .instruction();
...
async function getAccountData(programName, accountType, accountPubkey) {
    const loaded = loadedIDLs.get(programName);
    if (!loaded) {
        throw new Error(`Program ${programName} not loaded`);
    }
    ...
    try {
        ...
        return {
            data: JSON.parse(JSON.stringify(decoded, (_, v) => typeof v === 'bigint' ? v.toString() : v)),
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'load_idl': {
                const result = await loadIdl(args?.idl_path, args?.program_id);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'get_instruction_info': {
                const info = await getInstructionInfo(args?.program_name, args?.instruction_name);
                return { content: [{ type: 'text', text: info }] };
            }
            case 'generate_instruction': {
                const result = await generateInstruction(args?.program_name, args?.instruction_name, args?.args, args?.accounts);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'simulate_transaction': {
                const result = await simulateTransaction(args?.program_name, args?.instruction_name, args?.args, args?.accounts);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'get_account_data': {
                const result = await getAccountData(args?.program_name, args?.account_type, args?.account_pubkey);
                return {
                    content: [
                        {
                            type: 'text',
